...
function isActiveRunState(state: RunView['state'] | undefined | null): state is 'running' {
  ...
  const headerLabel = running
    ...
    : externallyRunning
      ...
      : run
        ? `Last run ${run.state}`
        : 'Idle';
  const showHeaderState = Boolean(app && (running || externallyRunning || view === 'arena' || view === 'fixtures'));
  useEffect(() => {
    onStatus(showHeaderState ? { label: headerLabel, tone: running || externallyRunning ? 'live' : '' } : null);
  }, [headerLabel, running, externallyRunning, showHeaderState, onStatus]);
