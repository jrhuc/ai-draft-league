  ...
  return {
    ...
    provider: {
      complete(system: string, messages: ProviderMessage[], options: CompleteOptions = {}): Promise<Completion> {
        calls.push({ system, messages: structuredClone(messages), options: structuredClone(options) });
...
test('own-build evidence lists what was left behind on the roster of the time, brought, and Mega Evolved', () => {
  ...
  try {
    ...
    const builds = [series.builds[series.entrants[0]]!, series.builds[series.entrants[1]]!] as const;
    ...
    const traded: WeeklyReviewSeries = { ...series, rosters: { ...series.rosters, 0: [BOARD.mons[30]!] } };
...
test('a coach keeps named pages, reads them back, and is refused an over-limit page with the reason', async (t) => {
  ...
  await runWeeklyReview(state, { runDir, psDir: defaultPsDir(), makeReviewProvider: () => week1.provider });
...
test('set_pages merges, delete_pages removes, and the reconciliation snapshot is addressable by stage', async (t) => {
  ...
  const week1 = scripted([reply('{"set_pages":{"lessons":"New lesson."},"delete_pages":["scouting"]}')]);
  await runWeeklyReview(state, { runDir, psDir: defaultPsDir(), makeReviewProvider: () => week1.provider });
...
test('a rejected reply is re-prompted with the reason and the attempt is logged', async (t) => {
  ...
  await runWeeklyReview(state, { runDir, psDir: defaultPsDir(), makeReviewProvider: () => script.provider });
