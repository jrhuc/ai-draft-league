...
test('coach trades validate both rosters and apply an accepted exchange atomically', () => {
  ...
  assert.deepEqual(
    ...
    { offer: { to: 1, give: 'charizard-mega-y', get: 'tyranitar', message: 'As listed.' }, reasoning: '' },
  );
  assert.match(
    String(
      parseTradeOffer('{"offer":{"to":0,"give":"charizard-mega-y","get":"tyranitar","message":"Self."}}', state, 0),
    ),
    /you are entrant 0/,
  );
...
test('the draft prompt states budget rules without computing a ceiling for the coach', () => {
  ...
  assert.match(prompt, /points to fill them from what is still on the board/, 'the budget rule is still stated');
  assert.ok(
...
test('a roster preset is refused when it breaks the board rules', () => {
  ...
  assert.deepEqual(octet.teams.slice(0, 4), preset.teams, 'the quartet is the first half of the octet');
  assert.throws(() => presetRosters(preset, board, 2), /4 teams for 2 entrants/);
  assert.throws(() => presetRosters({ ...preset, board: 'other' }, board, 4), /drawn from board other/);
