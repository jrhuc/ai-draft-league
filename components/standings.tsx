...
import { franchise, season } from "@/lib/load";
        ...
        <tbody>
          {season.standings.map((row) => (
            <tr key={row.franchiseId} className={row.rank === cut + 1 ? "playoff-line" : undefined}>
              ...
              {compact ? null : <td style={{ color: "var(--t4)" }}>{franchise(row.franchiseId).model.replace(/^[^:]*:/, "")}</td>}
              <td className="r num">{row.seriesWins}</td>
              <td className="r num">{row.seriesLosses}</td>
              <td className="r num">
                {row.gameWins}–{row.gameLosses}
              </td>
              <td className="r num" style={{ color: row.differential > 0 ? "var(--good)" : row.differential < 0 ? "var(--bad)" : "var(--t4)" }}>
