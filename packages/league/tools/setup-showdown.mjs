...
if (lock.repository !== repository || !/^[0-9a-f]{40}$/.test(lock.commit)) {
  throw new Error(
    "showdown.lock.json must contain the official repository URL and a full commit SHA",
  );
}
  ...
  try {
    for (const name of runtimePackages) {
      ...
      if (entry.hasInstallScript || Object.keys(dependencies).length) {
        throw new Error(
          `Review ${name}'s new install or runtime dependencies before updating Pokémon Showdown`,
        );
      }
      fs.cpSync(path.join(installedRoot, name), path.join(temporaryRoot, name), {
        recursive: true,
      });
    }
    ...
    for (const name of runtimePackages) {
      fs.cpSync(path.join(temporaryRoot, name), path.join(installedRoot, name), {
        recursive: true,
      });
    }
  } finally {
...
function main() {
  ...
  if (!fs.existsSync(directory)) {
    if (checkOnly)
      throw new Error("Pokémon Showdown is not installed; run pnpm run setup:showdown");
  ...
  if (updateMode && current !== lock.commit) {
    throw new Error(
      `Pokémon Showdown is at ${current || "an unknown revision"}; run pnpm run setup:showdown first`,
    );
  }
  ...
  if (current !== target) {
    if (checkOnly) {
      throw new Error(
        `Pokémon Showdown is at ${current || "an unknown revision"}; expected ${target}`,
      );
    }
    if (!updateMode)
      run("git", ["-C", directory, "fetch", "--depth=1", "--no-tags", lock.repository, target]);
