...
async function fetchSprites(): Promise<{
  written: number;
  substituted: string[];
  missing: string[];
}> {
...
async function fetchItemIcons(
  ...
  for (const stale of fs.readdirSync(SPRITE_DIR)) {
    if (stale.startsWith("itemicons-sheet-") && stale !== sheetName)
      fs.rmSync(path.join(SPRITE_DIR, stale));
  }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (substituted.length)
    console.log(`borrowed a base forme for ${substituted.length}: ${substituted.join(", ")}`);
