...
function checkFile(file) {
  ...
  try {
    ast = parse(text, {
      ...
      plugins: file.endsWith(".mjs")
        ? []
        : ["typescript", ...(file.endsWith(".tsx") ? ["jsx"] : [])],
      errorRecovery: true,
    });
  } catch (error) {
  ...
  for (const comment of ast.comments ?? []) {
    ...
    if (
      ...
      !/^[/!]?\s*(@ts-|eslint|biome-ignore|SAFETY:|#!|<reference)/.test(value)
    ) {
      violations.push(`${relative}:${comment.loc.start.line}: //${value.trim().slice(0, 90)}`);
    } else if (
      comment.type === "CommentBlock" &&
      !value.startsWith("*") &&
      !/^\s*biome-ignore/.test(value)
    ) {
      violations.push(
        `${relative}:${comment.loc.start.line}: /*${value.trim().split("\n")[0].slice(0, 90)}`,
      );
    }
  }
}
