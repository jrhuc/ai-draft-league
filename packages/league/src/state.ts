...
export class BattleState {
  ...
  compareActionOrder(args: JsonObject, reference: ShowdownReference): string {
    ...
    if (this.speciesKey(firstMove) === "encore") {
      const alreadyEncored = [...second.mon.volatiles].some(
        (volatile) => this.speciesKey(volatile) === "encore",
      );
  ...
  estimateDamage(args: JsonObject, request: BattleRequest, reference: ShowdownReference): string {
    ...
    if (!attackerName || !defenderName || !move)
    ...
    for (const [side, entry] of [
      ...
      if (side === "attacker") {
        ...
        if (ally.mon.item && !ally.mon.itemConsumed)
          authoritative.attacker_ally_item = ally.mon.item;
      } else {
        authoritative.defender_ally = ally.mon.species;
        if (allyAbility) authoritative.defender_ally_ability = allyAbility;
        if (ally.mon.item && !ally.mon.itemConsumed)
          authoritative.defender_ally_item = ally.mon.item;
      }
    }
    const screens = [...this.sides[defender.pid].conditions.keys()].filter((condition) =>
  ...
  private findMon(
    query: string,
  ): { pid: Pid; slot: number; mon: MonState; benched: boolean } | undefined {
  ...
  private findActive(query: string): { pid: Pid; slot: number; mon: MonState } | undefined {
    ...
    const candidates = prefixed
      ? this.activeEntries().filter(
          (entry) => (entry.pid === this.pid) === (prefixed[1] === "ally"),
        )
      : this.activeEntries();
    const wanted = prefixed ? prefixed[2]! : normalized;
    const exact = candidates.find(
      (entry) =>
        this.speciesKey(entry.mon.species) === wanted ||
        this.speciesKey(afterColon(entry.mon.ident)) === wanted,
    );
