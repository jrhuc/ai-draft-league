...
export function summarizeBattleEvents(lines: string[], pov?: "p1" | "p2"): string[] {
  ...
  for (const line of lines) {
    ...
    if (kind === "turn") summary.push(`Turn ${args[0]} begins.`);
      ...
      summary.push(
        `${ident(args[0])} entered as ${args[1]!.split(",", 1)[0]} at ${battleHpPercent(args[2])}.`,
      );
      ...
      if (ability && args[2] && blocked)
        summary.push(
          `${ident(blocked.slice(5))}'s ${args[2]} was blocked by ${ident(args[0])}'s ${ability[1]}.`,
        );
      else summary.push(`${ident(args[0])} could not act (${args[1]}).`);
    } else if (kind === "-status" && args.length >= 2)
      summary.push(`${ident(args[0])} became ${args[1]}.`);
    else if (kind === "-curestatus" && args.length >= 2)
      summary.push(`${ident(args[0])} was cured of ${args[1]}.`);
    else if (kind === "-ability" && args.length >= 2) {
      const copiedBy = args
        .find((arg) => arg.startsWith("[from] ability: "))
        ?.slice("[from] ability: ".length);
    ...
    else if (kind === "-miss" && args.length >= 2)
      summary.push(`${ident(args[0])} missed ${ident(args[1])}.`);
    else if (kind === "-prepare" && args.length >= 2)
      summary.push(
        `${ident(args[0])} is charging ${args[1]}; it releases next turn unless disrupted.`,
      );
      ...
      summary.push(`A critical hit landed on ${ident(args[0])}.`);
    else if (kind === "-supereffective" && args[0])
      summary.push(`The hit on ${ident(args[0])} was super effective.`);
      ...
      summary.push(
        `${ident(args[0])} ${args[1]} ${kind === "-boost" ? "rose" : "fell"} by ${args[2]}.`,
      );
    else if (kind === "-weather" && !args.includes("[upkeep]"))
      summary.push(`Weather became ${args[0] || "none"}.`);
      ...
      summary.push(
        pov && side
          ? `${side === pov ? "You" : "The opponent"} won the game.`
          : `${args[0]} won the game.`,
      );
