...
export class BattleTranscript {
  ...
  get lines(): string[] {
    ...
    const retainedFacts = this.facts.filter(
      (fact) => !this.timeline.some((line) => line.includes(fact)),
    );
    return [
      TRANSCRIPT_CLIP_MARKER,
      ...(retainedFacts.length ? [FACT_MARKER, ...retainedFacts] : []),
      ...this.timeline,
    ];
  }
  ...
  private rememberFact(value: string): void {
    ...
    this.factCharacters += value.length;
    while (this.factCharacters + this.facts.length - 1 > FACT_CHARACTER_LIMIT) {
      const removed = this.facts.shift();
      if (!removed) break;
      this.factCharacters -= removed.length;
    }
  }
