...
function seriesTable(rows: ParsedSeriesRecord[], limit = 100): string {
  const head =
  ...
  const body = rows
    ...
    .reverse()
    .map((row) => {
      const { games, score, teams } = row;
      const side = row.winner_side === "p1" || row.winner_side === "p2" ? row.winner_side : "";
      const clock = row.timer_scale === "off" ? "off" : `${row.timer_scale ?? 1}x`;
      return `<tr class='${side === "p1" ? "winner-a" : side === "p2" ? "winner-b" : ""}'><td>${escapeHtml(row.timestamp.slice(0, 19))}</td><td>${escapeHtml(row.mode)}</td><td>${escapeHtml(row.pool ?? "unrecorded")}</td><td>${escapeHtml(clock)}</td><td class=a>${escapeHtml(row.players.p1)}</td><td class=b>${escapeHtml(row.players.p2)}</td><td>${escapeHtml(`${teams.p1} vs ${teams.p2}`)}</td><td class=num>${score.p1}–${score.p2}</td><td>${escapeHtml(row.winner ?? "draw")}</td><td class=num>${games.length}</td><td class=num>${row.turns}</td></tr>`;
    })
    .join("");
  return `<h2>Recorded series</h2><div class=wrap><table>${head}${body}</table></div>`;
}

function gamesTable(rows: ParsedSeriesRecord[], limit = 80): string {
  const head =
    "<tr><th>Series</th><th>Game</th><th>p1</th><th>p2</th><th>Winner</th><th>Turns</th><th>Log</th></tr>";
  const games = rows.flatMap((series) => series.games.map((game) => ({ series, game })));
  const body = games
    .slice(-limit)
    .reverse()
    .map(({ series, game }) => {
...
export function writeReport(recordsPath: string, outPath: string, pool?: string): string {
  ...
  const document = `<!doctype html><meta charset=utf-8><title>AI Draft League records</title><style>${CSS}</style><h1>AI Draft League records</h1><p class=meta>${rows.length} completed series${poolText}. These are per-series outcomes from heterogeneous recorded contexts, not an aggregate model ranking. Pool, clock, opponents, and sample size remain attached to each row. Generated ${stamp} UTC.</p>${seriesTable(rows)}${gamesTable(rows)}`;
