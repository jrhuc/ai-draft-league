...
export class BattleLog {
  ...
  private push(
    kind: BattleLogKind,
    text: string,
    extra: Omit<BattleLogEntry, "turn" | "kind" | "text"> = {},
  ): void {
  ...
  private feedLine(line: string): void {
    ...
    if (kind === "turn" && args[0]) {
      ...
      this.push(
        "field",
        !args[0] || args[0] === "none" ? "Weather cleared" : `Weather: ${afterColon(args[0])}`,
      );
      ...
      this.push(
        "field",
        `${afterColon(args[1]!)} ${kind === "-sidestart" ? "up" : "ended"} on ${side}'s side`,
      );
      ...
      this.push(
        "timer",
        `${pid.toUpperCase()} ${TIMEOUT_TEXT.get(args[1] ?? "") ?? "timer event"}`,
      );
