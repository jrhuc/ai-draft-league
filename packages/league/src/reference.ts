...
export class ShowdownReference {
    ...
    for (const attacker of attackers) {
      ...
      for (const moveName of uniqueNames(attacker.moves)) {
        ...
        for (const defender of defenders) {
          ...
          if (blockedBy)
            bits.push(
              `${target.name} immune via ${blockedBy} (type chart ${effectivenessLabel(mod)})`,
            );
  ...
  lookup(name: string, args: JsonObject = {}): string {
    ...
    if (name === "calculate_stats")
      return this.calculateStats(calculateStatsArgumentsSchema.parse(args));
    if (name === "lookup_matchup") return this.lookupMatchup(matchupArgumentsSchema.parse(args));
    if (name === "estimate_damage")
      return this.estimateDamage(estimateDamageArgumentsSchema.parse(args));
  ...
  private calculateStats(args: CalculateStatsArguments): string {
    ...
    if (!species.exists)
      return `No species data for ${JSON.stringify(speciesName)} in ${this.format}.`;
    const nature = this.dex.natures.get(natureName);
    if (!nature.exists)
      return `No nature data for ${JSON.stringify(natureName)} in ${this.format}.`;
    ...
    const stats = this.battle.spreadModify(
      species.baseStats,
      statSet(this.battle, nature.name, evs, filledStats(31)),
    );
  ...
  private estimateDamage(args: EstimateDamageArguments): string {
    ...
    const [hpLow, hpHigh] =
      exactHp !== undefined ? [exactHp, exactHp] : hpRange(this.battle, defender.baseStats);
    ...
    for (const side of ["attacker", "defender"] as const) {
      ...
      if (ally)
        applied.push(`${side} ally ${ally.name}${ally.ability ? ` (${ally.ability})` : ""}`);
    }
