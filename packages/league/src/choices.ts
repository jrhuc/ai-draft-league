...
function switches(request: BattleRequest, reviving = false): SlotMenu {
  ...
  for (const [index, pokemon] of (request.side?.pokemon ?? []).entries()) {
    ...
    menu.push({
      label: `Switch to ${pokemonSpecies(pokemon)}`,
      part: `switch ${index + 1}`,
      kind: "switch",
    });
  }
...
function spreadLabel(target: string, slot: number, names?: TargetNames): string {
  ...
  if (target === "allAdjacent" || target === "all") {
    ...
    const species = names?.ally[ally];
    return species
      ? ` (all adjacent, including ally ${species})`
      : " (all adjacent, including ally)";
  }
  return " (spread)";
}
...
function moveItems(
  ...
  const protectHint =
    PROTECT_MOVES.has(name.toLowerCase().replace(/[^a-z0-9]+/g, "")) &&
    hints?.protectReduced?.[slot]
  ...
  return targets.flatMap(([targetPart, label]) => {
    ...
    const item: MenuItem = {
      label: `${name}${spread}${protectHint}${label}${warning}`,
      part,
      kind: "move",
    };
    return mega
      ? [item, { label: `${item.label} + Mega Evolve`, part: `${part} mega`, kind: "move" }]
      : [item];
  });
}
...
export function buildMenus(request: BattleRequest, hints?: MenuHints): SlotMenu[] {
  ...
  if (request.teamPreview) {
    ...
    const base = pokemon.map((mon, index): MenuItem => ({
      label: `Pick ${pokemonSpecies(mon)}`,
      part: String(index + 1),
      kind: "team",
    }));
  ...
  if (request.forceSwitch) {
    ...
    const targets = new Set(
      menus.flatMap((menu) =>
        menu.filter((item) => item.kind === "switch").map((item) => item.part),
      ),
    );
    if (targets.size < forced.length) {
      for (const index of forced) {
        const menu = menus[index]!;
        if (!menu.some((item) => item.kind === "pass"))
          menu.push({ label: "Pass", part: "pass", kind: "pass" });
      }
    }
    if (menus.some((menu) => menu.some((item) => item.kind === "switch")))
      menus[0]!.push({
        label: "Forfeit the game (concede the loss)",
        part: "forfeit",
        kind: "forfeit",
      });
    return menus;
