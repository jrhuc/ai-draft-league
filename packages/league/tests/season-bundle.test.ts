...
} from "../src/public/season-bundle.js";
    ...
    games: [
      {
        ...
        fielded: [[`pokemon-${sides[0]}`], [`pokemon-${sides[1]}`]],
        megaEvolved: [`pokemon-${sides[0]}`, null],
        faints: [{ [`pokemon-${sides[0]}`]: 1 }, {}],
      },
      {
        winner,
        turns: 7,
...
function fixture(): SeasonFixture {
  ...
  return {
    ...
    league: {
      ...
      series: [
        ...released,
        completedSeries(future.index, future.round, future.entrants, future.entrants[1]),
      ],
      teambuilds: [
        {
          seriesIndex: 0,
...
test("the season bundle publishes one complete week with its public evidence and nothing private or future", () => {
  ...
  assert.deepEqual(
    Object.keys(bundle.replays).sort(),
    released.map((series) => series.seriesId).sort(),
  );
  assert.equal(bundle.standings.filter((standing) => standing.seriesWins === 1).length, 2);
  const text = JSON.stringify(bundle);
  assert.equal(
    text.includes(PRIVATE_SENTINEL),
    false,
    "notebooks and unreleased reviews stay private",
  );
  ...
  assert.equal(
    bundle.weeks[0]?.matches[0]?.builds[0]?.sets,
    null,
    "closed sheets stay closed mid-season",
  );
  ...
  assert.deepEqual(bundle.weeks[0]?.matches[0]?.games[0]?.brought, released[0]?.games[0]?.brought);
  assert.deepEqual(
    bundle.replays[released[0]!.seriesId]?.games[0]?.faints,
    released[0]?.games[0]?.faints,
  );
...
test("a historical release replays only transactions inside its boundary", () => {
  const { league, plans } = fixture();
  const [first, second, third] = league.franchises;
      ...
      offers: [
        {
          ...
          message: null,
          accepted: true,
          offerReasoning: "The first released trade.",
          responseReasoning: "Accepted the first released trade.",
        },
      ],
      decisions: [],
    },
      ...
      offers: [
        {
          ...
          message: null,
          accepted: true,
          offerReasoning: "This trade is still future.",
          responseReasoning: "This response is still future.",
        },
      ],
      decisions: [],
    },
  ];
  first.roster = [third.draftRoster[0]!];
  second.roster = [first.draftRoster[0]!];
  third.roster = [second.draftRoster[0]!];
  const released = league.series.slice(0, 2);
  ...
  assert.deepEqual(
    bundle.franchises[0]?.roster.map(({ id, acquired, rationale }) => ({
      id,
      acquired,
      rationale,
    })),
    [{ id: "pokemon-1", acquired: "trade", rationale: "The first released trade." }],
  );
  assert.equal(JSON.stringify(bundle).includes("This trade is still future."), false);
});

test("the season bundle rejects a partial released week", () => {
  const { league, plans } = fixture();
...
test("the season bundle releases playoff rounds past the last week and reviews only at season end", () => {
  const { league, plans } = fixture();
  ...
  assert.equal(
    all.weeks[0]?.matches[0]?.builds[0]?.sets?.length,
    0,
    "closed sheets open once the season is over",
  );
