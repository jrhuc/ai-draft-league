...
test('the season bundle publishes one complete week with its public evidence and nothing private or future', () => {
  ...
  assert.deepEqual(bundle.replays[released[0]!.seriesId]?.games[0]?.faints, released[0]?.games[0]?.faints);
...
test('a historical release replays only transactions inside its boundary', () => {
  ...
  assert.deepEqual(
    bundle.franchises[0]?.roster.map(({ id, acquired, rationale }) => ({ id, acquired, rationale })),
