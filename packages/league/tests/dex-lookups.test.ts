...
test("fractional provider cost survives usage accumulation across tool rounds", async () => {
  ...
  const provider = scriptedProvider(
    [
      reply({
        toolCalls: [toolCall],
        usage: { input_tokens: 100, output_tokens: 50, cost: 0.031 },
      }),
      reply({
        text: '{"sets": []}',
        usage: { input_tokens: 200, output_tokens: 80, cost: 0.0205 },
      }),
    ],
    calls,
  );
  ...
  assert.ok(
    Math.abs((completion.usage.cost ?? 0) - 0.0515) < 1e-9,
    "sub-dollar costs must not truncate to zero",
  );
...
test("search_board is offered and dispatched only when a board search is supplied", async () => {
  ...
  const toolCall = {
    id: "call-1",
    name: "search_board",
    arguments: { learns: "Fake Out", max_cost: 20, limit: 5 },
  };
  ...
  assert.match(
    refused[0]!.result,
    /Not executed: tool "search_board" was not offered in this stage/,
  );
  ...
  assert.ok(
    draftTools.includes("search_board"),
    "the draft offers the board tool alongside the dex tools",
  );
...
test("extra tools are offered beside the dex tools and dispatched to their own runner", async () => {
  ...
  const provider = scriptedProvider(
    [
      reply({
        toolCalls: [{ id: "c1", name: "read_memory_page", arguments: { name: "lessons" } }],
      }),
      reply({ text: '{"sets": []}' }),
    ],
    calls,
