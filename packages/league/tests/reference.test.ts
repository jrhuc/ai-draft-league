...
test("reference reads exact data from the configured Showdown checkout", () => {
  ...
  assert.match(
    rendered,
    /Move Shadow Ball: Ghost; Special; BP 80; acc 100%; priority \+0; target normal/,
  );
...
test("lookup tools return one entry and reject missing data", () => {
  ...
  assert.deepEqual(DEX_TOOLS.find((tool) => tool.name === "lookup_species")!.parameters.required, [
    "name",
  ]);
  assert.equal(
    "level" in
      asRecord(DEX_TOOLS.find((tool) => tool.name === "lookup_species")!.parameters.properties),
    false,
  );
});
...
test("matchup and damage tools stay within open information", () => {
  ...
  assert.match(
    reference.lookup("lookup_matchup", { move: "Leaf Blade", defender: "Swampert" }),
    /2x|super-effective/,
  );
  assert.match(
    reference.lookup("lookup_matchup", { move: "Earthquake", defender: "Pelipper" }),
    /immune/,
  );
...
test("offline damage defaults to a single hit and honors an explicit spread flag", () => {
  ...
  assert.match(
    text(
      asRecord(
        asRecord(DEX_TOOLS.find((entry) => entry.name === "estimate_damage")!.parameters.properties)
          .is_spread_hit,
      ).description,
    ),
...
test("species lookups resolve board display names the dex does not alias", () => {
  ...
  assert.match(
    reference.lookup("lookup_species", { name: "Basculegion-Female" }),
    /Species Basculegion-F:/,
  );
  assert.match(reference.lookup("lookup_species", { name: "Meowstic-Male" }), /Species Meowstic:/);
  assert.match(
    reference.lookup("lookup_species", { name: "Mega Raichu Y" }),
    /Species Raichu-Mega-Y:/,
  );
  assert.match(
    reference.lookup("lookup_species", { name: "Mega Meowstic" }),
    /Species Meowstic-M-Mega:/,
  );
  assert.match(
    reference.lookup("lookup_species", { name: "Paldean Tauros" }),
    /Species Tauros-Paldea-Combat:/,
  );
  assert.match(
    reference.lookup("lookup_species", { name: "Paldean Tauros Aqua" }),
    /Species Tauros-Paldea-Aqua:/,
  );
  assert.match(
    reference.lookup("lookup_learnset", { name: "Basculegion-Male" }),
    /Learnset Basculegion \(/,
  );
  assert.match(
    reference.lookup("calculate_stats", {
...
test("damage estimates fall back to legal ranges for implausible stats but keep stage-boosted values", () => {
  ...
  const implausibleDef = reference.lookup("estimate_damage", {
    ...args,
    defender_stats: { hp: 1, def: 1 },
  });
  assert.match(
    implausibleDef,
    /defender_stats\.def 1 is implausible for Gengar-Mega, so the legal range was used/,
  );
...
test("damage estimates apply abilities, stages, burn, screens, and terrain through the engine", () => {
  ...
  const burned = maxPercent(
    reference.lookup("estimate_damage", { ...args, attacker_status: "brn" }),
  );
  assert.ok(near(burned / base, 0.5), `burn must halve physical damage (${base} -> ${burned})`);
  const boosted = maxPercent(
    reference.lookup("estimate_damage", { ...args, attacker_boosts: { atk: 2 } }),
  );
  ...
  assert.ok(
    near(screened / base, 2732 / 4096),
    `Reflect in doubles is a 1/3 cut (${base} -> ${screened})`,
  );
  const intimidated = maxPercent(
    reference.lookup("estimate_damage", { ...args, attacker_boosts: { atk: -1 } }),
  );
...
test("move lookups surface powder and sound interactions missing from descriptions", () => {
  ...
  assert.doesNotMatch(
    reference.lookup("lookup_move", { name: "Earthquake" }),
    /powder move|sound move/,
  );
});
...
test("compact reference shows the mega outcome for stone holders only", () => {
  ...
  const wrongStone = reference
    .renderCompact([
      { species: "Swampert", item: "Gengarite", nature: "Adamant", moves: [], active: false },
    ])
    .join("\n");
...
test("active allies apply their abilities without leaking their field effects", () => {
  ...
  const guarded = range(
    reference.lookup("estimate_damage", {
      ...quake,
      defender_ally: "Clefairy",
      defender_ally_ability: "Friend Guard",
    }),
  );
  const neutral = range(
    reference.lookup("estimate_damage", {
      ...quake,
      defender_ally: "Clefairy",
      defender_ally_ability: "Magic Guard",
    }),
  );
  ...
  assert.equal(
    range(
      reference.lookup("estimate_damage", {
        ...ball,
        attacker_ally: "Torkoal",
        attacker_ally_ability: "Drought",
      }),
    ),
  ...
  assert.equal(
    range(
      reference.lookup("estimate_damage", {
        ...sun,
        attacker_ally: "Clefairy",
        attacker_ally_ability: "Friend Guard",
      }),
    ),
