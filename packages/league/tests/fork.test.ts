  ...
  const surface = (battle: typeof live) => {
    ...
    return {
      ...
      requests: Object.fromEntries(
        pending.map((pid) => [pid, structuredClone(battle.getSide(pid).activeRequest)]),
      ),
      actions: Object.fromEntries(
        pending.map((pid) => [pid, requestActionCandidates(activeRequest(battle, pid))]),
      ),
    };
  };
...
test("a truncated choice list is reported rather than passed off as a finished game", () => {
  ...
  const replay = replayGame(
    { ...base, choices: { p1: choices.p1.slice(0, 1), p2: choices.p2.slice(0, 1) } },
    log,
  );
...
test("a forced replacement cannot pass every fainted slot while a reserve remains", () => {
  ...
  assert.deepEqual(requestActionCandidates(request).toSorted(), [
    "pass, switch 3",
    "switch 3, pass",
  ]);
});
...
test("a position reopens as a live battle that alternative actions can be played from", () => {
  ...
  const alternatives = requestActionCandidates(position.requests.p1).filter(
    (action) => action !== position.actual.p1,
  );
...
test("a position records what each side had actually been shown by then", () => {
  ...
  assert.ok(
    replay.positions.every(
      (entry, index) => index === 0 || entry.seen.p1 >= replay.positions[index - 1]!.seen.p1,
    ),
  );
...
test("the Showdown acceptance oracle rejects overgenerated candidates densely and deterministically", () => {
  ...
  const overgenerated = [
    rejected,
    ...candidates.map((entry) => ({ ...entry, number: entry.number + 50 })),
  ];
