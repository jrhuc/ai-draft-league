...
test("generic journals remain JSON objects while result authority requires the current series schema", (t) => {
  ...
  fs.writeFileSync(
    results,
    `${JSON.stringify({ ...row("a", "b", "a"), obsolete_identity: true })}\n`,
  );
  assert.deepEqual(loadSeriesRecords(results), [row("a", "b", "a")]);
});
...
test("record loading invalidates a cached torn tail after it is completed by an append", (t) => {
  ...
  assert.equal(
    loadSeriesRecords(records),
    cached,
    "an unchanged torn snapshot returns the cached valid prefix",
  );
