...
test("model discovery requires the OpenRouter key and Prime remains manual without a request", async () => {
  ...
  await assert.rejects(
    discoverModels(providerOption("openrouter")!, undefined, { fetch: fakeFetch }),
    /OPENROUTER/,
  );
  await assert.rejects(
    discoverModels(providerOption("opencode-go")!, undefined, { fetch: fakeFetch }),
    /OPENCODE/,
  );
  await assert.rejects(
    discoverModels(providerOption("prime")!, "prime-key", { fetch: fakeFetch }),
    /manual model IDs/,
  );
