...
import { franchiseName, monName, season } from "@/lib/load";
  ...
  return (
    <>
      <section className="hero">
        ...
        <p className="sub">
          Mid-season, each team may offer one trade and then swap from the undrafted pool. Each offer shows the message the model sent and the reasoning it kept private.
        </p>
      </section>
      {windows.length === 0 ? (
          ...
          <div className="grid">
            {window.offers.map((offer, i) => (
              <article key={`${offer.from}-${i}`} className="card card-pad offer">
                  ...
                  {offer.to ? (
                    <>
                      <span className="arrow">
                        offers {offer.give ? monName(offer.give) : "—"} for {offer.get ? monName(offer.get) : "—"} to
                      </span>
                      <TeamTag id={offer.to} />
                      {offer.accepted === true ? <span className="chip chip-good">Accepted</span> : offer.accepted === false ? <span className="chip chip-bad">Declined</span> : <span className="chip">No answer</span>}
                    </>
                  ) : (
                ...
                <div className="why">
                  <b>{franchiseName(offer.from)} privately:</b> {offer.offerReasoning || "—"}
                </div>
                {offer.to && offer.responseReasoning ? (
                  <div className="why">
                    <b>{franchiseName(offer.to)} privately:</b> {offer.responseReasoning}
                  </div>
                ) : null}
              </article>
            ))}
              ...
              <article key={move.franchiseId} className="card card-pad offer">
                ...
                {move.swaps.map((swap) => (
                  <div key={`${swap.drop}-${swap.add}`} className="swap">
                    <span className="drop">− {monName(swap.drop)}</span>
                    <span className="add">+ {monName(swap.add)}</span>
                  </div>
                ))}
                {move.reasoning ? (
                  <div className="why">
