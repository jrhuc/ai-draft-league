    ...
    <div className="section">
      <div className="filters" role="group" aria-label="Filter the board">
        <button
          type="button"
          className="filter"
          aria-pressed={filter.team === null && !filter.undrafted}
          onClick={() => setFilter({ ...filter, team: null, undrafted: false })}
        >
          All {board.length}
        </button>
        <button
          type="button"
          className="filter"
          aria-pressed={filter.undrafted}
          onClick={() => setFilter({ ...filter, team: null, undrafted: !filter.undrafted })}
        >
          ...
          <button
            ...
            onClick={() =>
              setFilter({
                ...filter,
                undrafted: false,
                team: filter.team === team.id ? null : team.id,
              })
            }
          >
      ...
      <div className="filters" role="group" aria-label="Filter by type">
        {TYPES.map((type) => (
          <button
            ...
            onClick={() => setFilter({ ...filter, type: filter.type === type ? null : type })}
          >
            {type}
          </button>
        ))}
      ...
      <div className="grid grid-6">
        {rows.map((mon) => {
          ...
          return (
            <article
              key={mon.id}
              className={`card hoverable board-mon${owner ? " taken" : ""}`}
              style={style}
            >
              ...
              <div>
                ...
                <div style={{ marginTop: "0.2rem" }}>
                  {mon.megaStone ? `${mon.megaStone}` : mon.abilities.join(" · ")}
                </div>
              </div>
              <div className="stats">
                {STATS.map((stat) => (
