...
type SideState = {
  preview: string[];
  slots: FieldSlot[];
  seen: Map<string, { hp: number; fainted: boolean }>;
};
    ...
    <div className="dec" style={toneStyle(team.tone)}>
      ...
      <span className="meta">
        ...
        {decision.reasoningTokens !== null
          ? `· ${tokens(decision.reasoningTokens)} reasoning tok`
          : ""}
      </span>
      {decision.rationale ? (
        <details>
...
function Game({
  ...
  game: ReplayGame;
  teams: [Team, Team];
  sprite: (species: string) => string | null;
}) {
  ...
  return (
    <div className="replay">
      <div className="field">
        {sides.map((side, index) => {
          ...
          return (
            <section
              key={team.id}
              className="card field-side"
              style={toneStyle(team.tone)}
              aria-label={`${team.name} field`}
            >
              <div className="who">
      ...
      <div className="transport">
        ...
        <button
          type="button"
          onClick={() => setCursor((value) => Math.min(total, value + 1))}
          disabled={finished}
        >
          Step
        </button>
        ...
        />
        <span className="pos">
          {finished ? "Final" : currentTurn === 0 ? "Team preview" : `Turn ${currentTurn}`} ·{" "}
          {cursor}/{total}
        </span>
      </div>
        ...
        {turns.map(({ turn, events }) => (
          <section key={turn} className="turn">
            ...
            {(decisionsByTurn.get(turn) ?? []).map(({ decision, position }) => (
              <DecisionRow
                key={`${turn}-${decision.franchiseId}-${position}`}
                decision={decision}
                team={teamFor(decision.franchiseId)}
                position={position}
              />
            ))}
            <div className="log">
